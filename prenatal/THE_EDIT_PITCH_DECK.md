# THE EDIT
## The wellness brand India was missing.

**Series Seed Deck**
**Aishwarya Chandrasekhar, Founder**
**April 2026**

---

## 01. The Problem

**What does an Indian woman take when she's planning a family?**

Today, her three real choices are:

- **Pharma-coded prescriptions** (Trimacare, Inatal, Pregomega) — feel like medicine, distributed by doctors, no brand connection
- **Imported wellness brands** (Ritual, Garden of Life, Pink Stork) — premium, but expensive, hard to access, no India context
- **Ayurvedic options** (Himalaya, Patanjali) — traditional framework, opaque sourcing, no transparency

**None of these were built for the modern Indian woman.**

---

## 02. The Insight — Why Now

The modern Indian woman has changed faster than the supplement market that's supposed to serve her.

- She **researches** before she buys — reads ingredient lists, looks up published studies
- She **expects transparency** — wants to know where ingredients come from, who formulated them
- She **expects modern branding** — wants packaging that belongs in her bathroom, not her first-aid kit
- She **wants control over her health journey** — preferring DTC subscription over pharmacy or doctor mediation
- She **buys on Instagram, expects 2-day delivery, reviews everything**

The Indian DTC wellness consumer is real and growing — but no prenatal/preconception/postnatal brand is built for her.

---

## 03. The Solution

**The Edit is built for her.**

A modern, transparent, gynaecologist-co-formulated brand for Indian couples and parents-to-be — across every stage of the family-building journey.

> Every ingredient traceable to a named source. Every dose backed by published science. Every formula co-designed with practicing gynaecologists. Built in India. Made for the moments that matter most.

**Brand promise:** Only what matters. For the stage you're in.

---

## 04. The Product

Three SKUs covering the full family-building journey:

### Edit For Two (Hero SKU)
Couples preconception bundle. One box, two bottles. *Edit For Her* (egg quality, fertility) + *Edit For Him* (sperm quality, antioxidants).

### Edit Prenatal
Comprehensive 18–25 ingredient prenatal supplement. Branded traceable ingredients (Quatrefolic methylfolate, life'sDHA algal DHA, Albion chelated minerals). All within ICMR RDA.

### Edit Postnatal *(Year 1 expansion, Month 9–12)*
Postpartum recovery + lactation support.

**Pricing:** Rs.800–1,500/month per SKU. Couples bundle priced ~15% below buying separately.

---

## 05. Why The Edit Wins — The Competitive Gap

**No Indian brand combines all seven of these. The Edit will.**

| Capability | Trimacare | Wellbeing Nutrition | Imported (Ritual, etc.) | Ayurvedic | **The Edit** |
|---|---|---|---|---|---|
| DTC subscription | ❌ Doctor-channel | ✅ | ✅ Imported | ❌ Pharmacy | ✅ |
| Modern lifestyle branding | ❌ Pharma-coded | ✅ | ✅ | ❌ | ✅ |
| Traceable branded ingredients | ⚠️ Some | ⚠️ | ✅ | ❌ Opaque | ✅ |
| Gynaecologist co-formulation | ✅ | ❌ | ❌ | ❌ | ✅ |
| Full family-building journey | ❌ Prenatal only | ❌ Not focused | ⚠️ Some | ❌ | ✅ |
| Published-science claims | ✅ | ⚠️ | ✅ | ❌ | ✅ |
| Made in India + accessible price | ✅ | ✅ | ❌ | ✅ | ✅ |

**This is the white space.**

---

## 06. The Market Opportunity

**India is the largest reproductive health market in the world that nobody is building intentionally for.**

| | |
|---|---|
| **TAM** — Indian health & wellness market | $164B (2025) → $258B (2034), 5.14% CAGR |
| **SAM** — Indian prenatal + women's wellness supplements | Rs.5,000 Cr+, growing 12% CAGR |
| **SOM** — Premium DTC capture (3 years) | Rs.50–100 Cr annual revenue |
| **Customer base** | 27M pregnancies/year + 5–8M couples actively TTC |
| **Asia-Pacific share of global postpartum market** | 38.58% (largest regional share) |

---

## 07. Business Model

**Premium DTC subscription. Long customer lifecycle. Couples-driven AOV.**

- **Pricing:** Rs.1,200/month per SKU (avg)
- **Couples Bundle (hero):** Rs.2,000/month — 2x AOV at zero new acquisition cost
- **Customer journey:** 15+ months (Preconception → Pregnancy) at launch; **27+ months once Postnatal launches**
- **Gross margin target:** 70–75%
- **CAC target:** Rs.800–1,500 declining as brand awareness grows
- **LTV / CAC:** 5x+ over the full journey

---

## 08. Go-to-Market

**Multi-channel acquisition, content-led trust building.**

- **Performance marketing** — Meta + Google ads, urban metro 28–38 demographic
- **Influencer collaborations** — mom-creators, wellness creators, practicing gynaecologists with social presence
- **Gynaecologist network** — leveraging the co-founder's clinical credibility for medical referrals
- **PR + content marketing** — owning the "transparency" + "couples" narrative
- **Editorial partnerships** — wellness publications, Femina, Cosmo, women's media

**Distribution:** Direct from theedit.in (Shopify). Subscription-first, with one-time purchase option.

---

## 09. Early Validation

*Where we are today (April 2026):*

- ✅ **Brand thesis locked** — manifesto, positioning, product architecture defined
- ✅ **Manufacturer discovery underway** — Tanishq Lifecare + Brukem Life Care in active conversations
- ✅ **Customer validation in progress** — *[X interviews completed, Y waitlist signups]*
- ✅ **Regulatory pathway clarified** — FSSAI FSDU + Health Supplement endorsements
- ⏳ **Gynaecologist co-founder search** — 5–10 candidates being approached
- ⏳ **Trademark filing** — IP India Class 5 + 30 search underway

*[Note for VCs: full data room available on request including customer interview transcripts, manufacturer LOIs, regulatory consultant assessment]*

---

## 10. The Team

### Aishwarya Chandrasekhar — Founder & CEO
Founded The Edit after personally experiencing the trust gap during her own family-building journey. Background in [to be filled]. The brand's voice and product vision.

### Thambusamy T D (Thambu) — Co-Founder & Strategic Advisor
Founder of Lumen Marketing Company (CDRI-08 cognitive supplement, June 2026 launch) and RaceSims Solutions Pvt Ltd (India's pro-grade sim racing hardware). Two-decade track record across consumer + premium hardware brands. Brings deep supplements industry knowledge and operational scaling expertise.

### Gynaecologist Co-Founder — *To Be Named*
Practicing OB-GYN co-founder leading clinical formulation. Search underway; expected to be announced before launch. Will hold founder-level equity, not advisory.

### Founding Advisors — *To Be Named*
Industry advisors with DTC and consumer brand experience.

---

## 11. The Ask — Pre-Seed Round

**Raising Rs.75–90L (~$90K–110K) at a [valuation TBD] pre-money valuation.**

### Use of funds (12-month runway)

| Category | Allocation |
|---|---|
| Day 1 inventory (3 SKUs × MOQs) | Rs.12–15L |
| Marketing (performance + influencer + PR) | Rs.30–40L |
| Brand identity, packaging, content production | Rs.4–6L |
| FSSAI consultant + regulatory filings | Rs.3–4L |
| Tech + operations + working capital | Rs.5–8L |
| Founder/team runway (12 months) | Rs.10–15L |

### 12-month milestones

- ✅ FSSAI FSDU + Health Supplement licenses approved
- ✅ Edit For Two + Edit Prenatal launched commercially
- ✅ 5,000+ paying customers
- ✅ Rs.5–8 Cr ARR run rate
- ✅ Edit Postnatal launched (Month 9–12)
- ✅ Series A preparation begins

---

## 12. The Vision

**The Edit is a women's lifecycle wellness platform — not a prenatal company.**

The brand walks with Indian families through every major stage of life:

```
Preconception ─► Pregnancy ─► Postpartum ─► Perimenopause ─► Menopause
   (Year 1)      (Year 1)     (Year 1)        (Year 3)        (Year 5)
```

Year 3–5 expansion into postpartum recovery, perimenopause, menopausal transition, and supplements for the next generation.

**This is a Rs.500 Cr+ brand trajectory** — built on the most defensible, most underserved demographic in Indian wellness: the modern Indian woman, across every stage of her family-building life.

---

## 13. Why Now. Why This Team. Why The Edit.

**Why now:** The modern Indian DTC consumer exists, the regulatory framework (FSSAI FSDU) is clear, supply chain partners are mature, and no premium brand owns this space.

**Why this team:** Founder lived the problem. Co-founder has built consumer + supplement brands at scale. Gynaecologist co-founder brings clinical credibility no aggregator competitor can match.

**Why The Edit:** Brand-led, gynaecologist-formulated, transparency-first, India-specific. The wellness brand the largest reproductive health market in the world has been waiting for.

---

## Contact

**Aishwarya Chandrasekhar**
Founder, The Edit Pvt Ltd (incorporation in progress)

📧 [contact email]
📱 [contact phone]
🌐 theedit.in (launching soon)
