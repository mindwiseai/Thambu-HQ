---
title: RaceSims · Chennai (Flagship Centre)
type: concept
domain: racesims
status: PLANNING
tags: [venue, flagship, chennai, direct-operation, capex]
updated: 2026-05-18
---

# RaceSims · Chennai (Flagship Centre)

The direct-operated premium sim racing centre opening in Chennai under [[racesims-company|RaceSims Solutions Pvt Ltd]]. **Centre name on the door: "RaceSims · Chennai"** (low-friction, casual-funnel-friendly). **The Motorsport Academy** lives inside as the structured upsell programme. **Total Y1 investment ₹95L (₹72L hard capex + ₹22L working capital)**, monthly opex ₹4.34L (Y1), **Y1 EBITDA ₹15.1L, 5-year cumulative EBITDA ₹5.16cr, 25-month payback, 5.4× capital multiple, Y5 EBITDA margin 68%**. Includes Track Day Programmes revenue stream (Chennai-specific HNI supercar coaching). Serves as the proof point and franchise prototype for the [[racesims-india-saturation-plan|43-centre India saturation plan]]. Latest v2 proposal: [[2026-05-18-chennai-flagship-proposal-v2]]. Brand architecture: [[racesims-naming-architecture]].

> [!important] Numbers basis (2026-05-31) — **capital source of truth = `racesims-chennai-project-report.pdf`** (room-only, landed): net **₹68.46L** · break-even **11.5%** · **~3.1×** · 5-yr EBITDA ₹3.57cr / PAT ₹2.11cr. **This flagship page is the broader UPSIDE model** — it layers in café, Academy and Track-Day revenue and uses distributor-cost equipment (₹5.84L/rig as a retail-equity lens), so its larger headline figures (₹95L all-in, ₹5.16cr 5-yr, 5.4×, 25-mo payback) are the *with-upside* scenario, **not** the committed capital. For any capital / break-even / multiple reporting, cite the project report. See [[racesims-chennai-premium-baseline]] (conservative floor) — reconcile, don't conflate.

## Naming architecture (locked May 18)

The centre's customer-facing name is **"RaceSims · Chennai"** — *not* "RaceSims Motorsport Academy." [[racesims-motorsport-academy|The Motorsport Academy]] is the structured **programme inside**, not the centre's name. The unbundle preserves the casual funnel (college kids already calling about drop-in racing) while reserving Academy legitimacy for serious aspirants. Full reasoning: [[racesims-naming-architecture]].

| Surface | Name | Audience |
|---------|------|----------|
| Storefront / Google / Instagram bio | **RaceSims · Chennai** | Anyone — walk-in friendly |
| Casual access | **Open Sessions** | Drop-ins, college kids, dates, groups |
| Recurring access | **Pass 8 / Pass 16** | Regulars |
| Group experiences | **Experiences** (Team Day, Birthday, Corporate) | B2B, parties |
| Career-track training | **[[racesims-motorsport-academy|The Motorsport Academy]]** | Aspirants, parents, sponsors |
| Competitive arc | **RaceSims Championship** | Competitors, fans |
| Sim-to-seat pathway | **[[racesims-the-ladder|The Ladder]]** | Top graduates → ARKA / Polo Cup |

> [!contradiction]
> **Numbers revised from v1 (May 17) to v2 (May 18).** The May 17 planning conversation had ₹66-68L investment / ₹4.97L opex / ₹32L Y1 EBITDA / 17-month payback. The May 18 deep-dive rebuild replaces those with: ₹92L investment / ₹4.63L opex / ₹12.1L Y1 EBITDA / 26-month payback. **Why the change:**
> - Rig spec upgraded from generic Moza-tier estimates (₹5.03L/seat) to actual Conspit/VNM kit at distributor cost (Rally ₹5.85L, Motion ₹9.85L) per Excel SKUs
> - Infrastructure rebuilt line-by-line (was bundled estimates) → ₹27L vs. earlier ₹15-18L
> - Utilisation revised from aggressive 72% Y5 to base-case 68% (industry benchmark 50-65% for mature entertainment venues)
> - Merch attach corrected from ~10% daily to realistic 3-4% walk-in attach
> - Director ₹75K/month removed from this centre P&L (HQ-level cost, not centre)
> - Y1 ancillary corrected — was modelling 50%+ of rig revenue (unrealistic for Y1), now 14% Y1 → 28% Y5
> v2 numbers are honest, defensible, investor-grade. **v2 is authoritative going forward.**

> [!contradiction]
> **Seat count discrepancy.** The executed floor plan (May 29 layout, L-shape 8 rigs) has **8 seats: 7 rally + 1 motion**. The P&L model and capex table below were built on a 6-rig assumption (5 rally + 1 motion). Capacity ceiling, capex, and all P&L numbers need recalibration for 8 rigs. See [[racesims-master-rate-card]] for current seat config.

## What this is

A 2,000 sqft premium sim racing venue with 8 rigs (7 Rally + 1 Motion), café-lounge, viewing/event area, and event-streaming kit. Not a gaming café — a professional motorsport development facility positioned via [[racesims-drive-train-compete|Drive · Train · Compete]] with a clear pathway from walk-in customer to real-motorsport seat via [[racesims-the-ladder|the Ladder]].

## Capital investment — v2 (locked)

### Hard capex · ₹69.32L

| Component | ₹ |
|-----------|---|
| Rigs · 5 Rally @ ₹5,84,900 + 1 Motion @ ₹9,84,900 | 39,09,400 |
| Space prep (HVAC, interior, flooring, lighting, furniture) | 8,50,000 |
| Power & electrical (wiring + UPS 6 KVA) | 2,50,000 |
| IT, software, networking, POS | 2,55,000 |
| Brand & CX (signage, café setup, event/streaming kit) | 4,60,000 |
| Safety, spares & tools | 1,08,000 |
| Deposits & pre-opening (incl. 6 mo rent deposit ₹9L) | 11,00,000 |
| **Hard capex** | **69,32,400** |

### Working capital · ₹23.13L
5 months × ₹4.63L monthly opex

### **Total Y1 cash need · ₹92.45L**

### Built-in equity
Rigs deployed at our cost (₹39L) are worth **₹65-70L at retail**. Day-one balance-sheet equity of ₹26-31L per centre — the [[conspit]] / [[vnm]] distributor moat.

### Per-rig spec (Conspit/VNM at distributor cost)

| Component | Rally Rig | Motion Rig |
|---|---|---|
| Gaming PC (RTX 4070Ti+) | ₹2,70,000 | ₹2,70,000 |
| Monitor (premium tier) | ₹75,000 | ₹75,000 |
| Cockpit frame | ₹35,000 | ₹35,000 |
| Bucket seat | ₹20,000 | ₹20,000 |
| DD wheelbase | ₹42,000 | ₹42,000 |
| GT/rally wheel | ₹28,000 | ₹28,000 |
| QR + load-cell pedals | ₹34,900 | ₹34,900 |
| Handbrake + shifter | ₹40,000 | ₹40,000 |
| Audio + speakers | ₹40,000 | ₹40,000 |
| Motion system | — | ₹4,00,000 |
| **Per rig** | **₹5,84,900** | **₹9,84,900** |

## Monthly opex — v3.1 LOCKED · ₹4.34L (Y1)

Material revisions May 19: **rent locked at ₹1.75L** (bump from ₹1.25L for conservative budgeting on premium 2,000 sq ft location). Electricity ₹50K (5-star ACs + realistic PC load curves), maintenance ₹7.5K, software ₹3K (Zoho + Workspace billed elsewhere), music licence removed, insurance ₹7.5K (proper ₹50L+ asset cover), payment gateway as 2% of digital revenue, marketing ₹35K Y1 launch then taper to ₹25K from Y2.

### Team (4 direct + outsourced CA)

| Role | ₹/month |
|---|---|
| Senior Team Lead (also handles Zoho Books) | 35,000 |
| 2 × Multi-role Team Member (sim ops + floor + marketing/content) | 40,000 |
| Housekeeping | 5,000 |
| Statutory loading (PF + ESI + bonus ~28%) | 22,400 |
| Outsourced CA (filings only) | 7,000 |
| **Staff total** | **1,09,400** |

No security, no in-house accountant/marketing, no centre-manager / sim-coach split.

### Overheads (Y1)

| Item | ₹/month |
|---|---|
| Rent (2,000 sq ft commercial, 6-mo deposit) | 1,75,000 |
| Electricity (4 × 1.5T 5-star ACs + realistic PC) | 50,000 |
| Internet + water + utilities | 5,000 |
| Maintenance & spares | 7,500 |
| Software (booking, race-mgmt; Zoho/Workspace elsewhere) | 3,000 |
| Cleaning supplies & consumables | 3,000 |
| Marketing (Y1 launch · taper to ₹25K Y2+) | 35,000 |
| Incentive pool reserve | 25,000 |
| Insurance (₹50L+ asset + public liability) | 7,500 |
| Payment gateway (Y1 · 2% of digital revenue) | 8,400 |
| Misc / contingency | 5,000 |
| **Non-staff total** | **3,24,400** |
| **TOTAL MONTHLY OPEX Y1** | **4,33,800** |

Café COGS + merch COGS modelled as variable (~35% café revenue / ~40% merch revenue), added separately in the P&L.

## Revenue model — v2

### Capacity ceiling (immovable)
**26,280 seat-hours/year** (6 rigs × 12 hr × 365 days). Hard cap on rig revenue.

### Utilisation ramp (base case)

| Year | Util | Hrs/rig/day | Blended ₹/hr |
|---|---|---|---|
| Y1 · 2026 | 30% | 3.6 | ₹750 |
| Y2 · 2027 | 50% | 6.0 | ₹800 |
| Y3 · 2028 | 60% | 7.2 | ₹900 |
| Y4 · 2029 | 65% | 7.8 | ₹1,000 |
| Y5 · 2030 | 68% | 8.2 | ₹1,100 |

Break-even at 28% util; Y1 lands just above at 30%.

### Rate card (locked May 29 — see [[racesims-master-rate-card]] for full detail)

| Product | Rate |
|---|---|
| 30-minute session | ₹750 |
| 1-hour session | ₹1,250 |
| Motion rig surcharge | +₹500/hr |
| Pass 8 (8 hrs/month) | ₹8,000/mo |
| Pass 16 (16 hrs/month) | ₹14,000/mo |
| Team Day (4–6 seats · 3 hrs flat) | ₹20,000 |
| Private Centre Hire | ₹8,000/hr (3-hr min) |
| Initiation Programme | ₹8,000 (4 × 1hr coached) |
| Race Academy | ₹20,000 (8 × 1hr coached) |
| Rally School | ₹25,000 (10 × 1hr coached) |
| Championship Pass (per discipline) | ₹4,500/yr |

### Peak vs off-peak saturation (Y5)
- Weekday off-peak (210 hrs/wk): ~45% util · room to grow
- Weekday peak (150 hrs/wk): ~90% util · near full
- Weekend (144 hrs/wk): ~88% util · near full
- **Centre #2 trigger: weekday peak ≥90% for 3 consecutive months** (expected mid-Y3)

### Off-peak strategy
Corporate team-building, coaching/race academy, schools/colleges (esports clubs), league play (championship qualifiers), influencer/content shoots.

### Ancillary streams (grows 14% → 28% of rig revenue)
- **Café & F&B** ~50% (60-65% attach, ₹150-250 avg ticket)
- **Events & corporate** ~25% (corporate ₹40-60K, birthdays ₹15-25K, private nights ₹30-50K)
- **Coaching & academy** ~8% (1-on-1, masterclasses, pro driver tie-ups via [[arka-motorsports]])
- **Championship entries** ~5% (from 2027)
- **Merchandise** ~6-9% (realistic 3-4% walk-in attach + event-day spikes)

## 5-year P&L (base case, ₹L) — v3.2 LOCKED (₹1.75L rent + Track Day Programmes)

| | Y1 | Y2 | Y3 | Y4 | Y5 | **5Y Total** |
|---|---|---|---|---|---|---|
| Rig revenue | 59.1 | 105.1 | 141.9 | 170.8 | 196.6 | 673.5 |
| Ancillary (café, events, Academy, Real Drives, merch) | 8.3 | 19.5 | 33.2 | 45.1 | 55.1 | 161.2 |
| Track Day Programmes (NEW · supercar coaching) | 1.5 | 3.0 | 5.0 | 7.0 | 10.0 | 26.5 |
| **Total Revenue** | **68.9** | **127.6** | **180.1** | **222.9** | **261.7** | **861.2** |
| Opex (incl. café/merch/Track Day COGS) | 53.8 | 62.3 | 69.5 | 76.3 | 83.2 | **345.1** |
| **EBITDA** | **15.1** | **65.3** | **110.6** | **146.6** | **178.5** | **516.1** |
| EBITDA margin | 22% | 51% | 61% | 66% | 68% | 60% |
| Depreciation | 10.9 | 10.9 | 10.9 | 10.9 | 10.9 | 54.3 |
| PBT | 4.2 | 54.4 | 99.7 | 135.7 | 167.6 | 461.6 |
| Tax @ 25% | 1.1 | 13.6 | 24.9 | 33.9 | 41.9 | 115.4 |
| **PAT** | **3.2** | **40.8** | **74.8** | **101.8** | **125.7** | **346.2** |

### Opex line detail (Y1 → Y5, ₹L)

| Line | Y1 | Y2 | Y3 | Y4 | Y5 | 5Y |
|---|---|---|---|---|---|---|
| Payroll + statutory + CA | 13.1 | 17.2 | 18.2 | 19.3 | 20.4 | 88.2 |
| Rent (₹1.75L base · 5% escalation) | 21.0 | 22.1 | 23.1 | 24.3 | 25.5 | 116.0 |
| Electricity & utilities | 6.6 | 7.5 | 8.3 | 8.8 | 9.2 | 40.4 |
| Maintenance & spares | 0.9 | 1.0 | 1.1 | 1.1 | 1.2 | 5.3 |
| Marketing (₹35K Y1, ₹25K from Y2) | 4.2 | 3.0 | 3.2 | 3.4 | 3.6 | 17.4 |
| Incentive pool | 3.0 | 3.2 | 3.4 | 3.6 | 3.8 | 17.0 |
| Insurance | 0.9 | 1.0 | 1.0 | 1.1 | 1.2 | 5.2 |
| Payment gateway (~2% of digital) | 1.0 | 1.9 | 2.8 | 3.5 | 4.3 | 13.5 |
| Café COGS + merch COGS | 1.7 | 3.8 | 6.6 | 9.1 | 11.6 | 32.8 |
| Software, internet, cleaning, misc | 1.3 | 1.4 | 1.5 | 1.7 | 1.8 | 7.7 |
| **Total opex** | **53.7** | **62.1** | **69.2** | **75.9** | **82.6** | **343.5** |

## Headline metrics (v3.2 LOCKED)

| Metric | Value |
|---|---|
| Y1 cash need | **₹95L** (₹72L capex + ₹22L WC) |
| Y3 EBITDA | ₹1.11 cr |
| Y5 EBITDA | ₹1.79 cr |
| 5Y cumulative EBITDA | **₹5.16 cr** |
| 5Y cumulative PAT | ₹3.46 cr |
| EBITDA on capital multiple | **5.4×** |
| Payback period | **~25 months** (Q1 of Y3) |
| Y5 EBITDA margin | **68%** |
| Break-even utilisation | **~23% blended** (Y1 plan 30% → 7-pt cushion) |

## Cumulative cash position (₹L)

| Year | EBITDA | Cumulative | Net of ₹95L |
|---|---|---|---|
| Y1 | 15.1 | 15.1 | −79.9 |
| Y2 | 65.3 | **80.4** | −14.6 |
| Y3 | 110.6 | 191.0 | **+96.0** |
| Y4 | 146.6 | 337.6 | +242.6 |
| Y5 | 178.5 | 516.1 | +421.1 |

Capital fully returned by Q1 of Y3. Pure compounding thereafter.

## Sensitivity scenarios

| Scenario (Y5 util) | 5Y EBITDA | Payback |
|---|---|---|
| Conservative · 62% | ₹4.4 cr | ~28 mo |
| **Base case · 68%** | **₹5.16 cr** | **~25 mo** |
| Aggressive · 72% | ₹5.55 cr | ~23 mo |

## Strategic role

1. **Operating business** — generates ₹12L Y1 → ₹165L Y5 EBITDA
2. **Franchise prototype** — every process documented as the playbook for the [[racesims-india-saturation-plan|43-centre network]]
3. **Brand proof point** — first place in India where sim racing is positioned as motorsport infrastructure
4. **Championship venue** — hosts qualifiers for [[racesims-championship-architecture|National Sim Racing Championship 2027]]

## Pre-launch milestones (unchanged)

| Week | Action |
|------|--------|
| 1-2 | Scout 5 spaces across Anna Nagar / Kilpauk / Alwarpet |
| 1 | Brief CA on running service + retail under same PAN |
| 4 | Hardware order placed (8-week lead time) |
| 4-6 | Lease signed (negotiate 6-month deposit, 2-3 month rent-free fit-out) |
| 5-6 | Senior Team Lead hired |
| 6-12 | Fit-out + installation |
| 12 | Hardware arrives, install begins |
| 13-14 | Systems commissioning, staff training |
| 15 | Soft launch (invite only, 30 people) |
| 16 | **Public opening** |

## Output artefacts
- **HTML deck:** `racesims/marketing/decks/chennai-centre-proposal.html` (11 slides, RaceSims editorial theme — cream + RS Red + Montserrat)
- **PDF deck:** `racesims/marketing/decks/chennai-centre-proposal.pdf`
- **Source page:** [[2026-05-18-chennai-flagship-proposal-v2]]

> [!contradiction]
> A conservative counter-model, [[racesims-chennai-premium-baseline]] (2026-05-29), assumes **18% Year-1 occupancy** — anchored to real dealer [[uday-hyderabad|Uday]]'s ~20%-at-month-6 — vs this page's 30→68% ramp. The deck's occupancy is 1.5-3× the only real network data point. Use the baseline as the conservative floor; this page is the upside case (academy/café/Ladder funded).

## Connections

- [[racesims-chennai-premium-baseline]] — **conservative sim-only baseline (Uday-anchored, more pessimistic occupancy)**
- [[2026-05-18-chennai-flagship-proposal-v2]] — **v2 source (authoritative)**
- [[racesims-naming-architecture]] — **brand & naming architecture (centre vs programme split)**
- [[racesims-motorsport-academy]] — **the structured training programme inside the centre**
- [[2026-05-17-racesims-chennai-flagship-planning]] — v1 source (superseded numerically; concepts/strategy still valid)
- [[racesims-drive-train-compete]] — positioning framework
- [[racesims-championship-architecture]] — championship hosted here
- [[racesims-the-ladder]] — sim-to-reality pathway
- [[racesims-vms-build]] — custom software platform
- [[racesims-india-saturation-plan]] — multi-centre rollout this validates
- [[racesims-revenue-stack]] — 6 adjacencies layered on top
- [[racesims-company]] — operating entity
- [[conspit]] + [[vnm]] — the distributor moat enabling the cost edge
- [[arka-motorsports]] — partner for Rally Pathway + pro driver coaching
- [[thambu]] — director
- [[domains/racesims/_index]]
