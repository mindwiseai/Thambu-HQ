---
title: April 2 Mumbai In-Person Meeting
type: concept
domain: mindwise
status: PAST (travel confirmed, meeting outcomes still pending ingestion)
tags: [meeting, milestone, in-person]
---

# April 2, 2026 — Mumbai In-Person Meeting

**Target date:** Wednesday, April 2, 2026
**Travel window confirmed:** Mar 31 → Apr 2, 2026 (per [[thambu]]'s email March 27: *"I've decided to travel to Mumbai from the 31st to the second"*)
**Location:** [[sorted-agency|Sorted Agency]] / [[manik|Manik]]'s office, Mumbai
**Attendees:** [[thambu]], [[shivendra-selvam]], [[manik]], potential performance/social media agency person
**Note:** virtual link offered for Thambu's father to join/drop

This was the **single most important checkpoint** in the [[launch-plan-june-1-2026|June 1 launch plan]] — the gate that unlocks the [[critical-path-bottleneck|critical path]].

## ✅ What we know from email ingestion (2026-04-12)

Two email findings from the Gmail ingest confirm the trip structure:

1. **Travel confirmed:** [[thambu]] emailed on March 27 confirming he'd travel Mumbai Mar 31 → Apr 2.
2. **Three-reason trip bundle:** per the [[2026-03-26-thambu-voice-brief|March 26 voice brief]], the Mumbai leg of the trip was bundled around **three independent reasons**:
   - [[jay-kularia]] motion system installation (RaceSims customer obligation — the anchor that fixed the dates)
   - [[patco-pharmaceuticals|Patco Packing Solutions]] — finalize Mindwise bottle design
   - [[imcd]] — explore other extracts, including [[beta-free-bacopa-extract]]

   The Sorted visual identity presentation was the **brand side** of the trip. The voice brief never explicitly mentioned the visual identity presentation — suggesting the trip might have been planned more around the customer install + supplier meetings, with the Sorted session plugged into the same days.

⚠️ **Still outstanding:** the actual **outcomes** of the April 2 meeting are still not in the wiki. This remains the highest-priority next ingestion. Possible sources:
- WhatsApp messages from Sorted dated April 2-5 (not yet ingested — WhatsApp access is manual)
- Email from Manik / Binita / Aarti / Sorted after the meeting (none found in Gmail search — they communicate via WhatsApp, not email)
- Follow-up voice brief from Thambu post-April 2 (none found yet — if one exists, check `voice-briefs/`)
- A status note written by Thambu reflecting on the meeting

## Planned agenda

1. **Visual Identity Presentation** — [[sorted-agency|Sorted]] presents the full brand book, design language, and visual system. Team reviews and provides feedback. See [[route-3-architecture-of-intelligence]].
2. **Website Discussion** — using the visual identity as context, discuss whether to proceed with Sorted at ₹6L, negotiate, or explore external developers. See [[website-development]].
3. **Meet Performance/Social Media Agency** — face-to-face introductions with the freelancer Manik was vetting. Discuss scope (performance ads + partial social media), pricing, timeline. See [[brand-manager-decision]].
4. **Packaging Status** — quick update on packaging brief revisions and render timeline. See [[two-track-bottle-strategy]] and [[2026-03-23-binita-packaging-feedback]].
5. **PR Connect** — confirm freelancer PR contact and next steps. See [[freelance-pr-strategy]].

## Required deliverables before April 2

| # | Deliverable | Owner | Deadline |
|---|---|---|---|
| 1 | Visual Identity Presentation (brand book, design language, logo applications, color, typography) | Sorted (Binita / Manik) | By March 30 - April 1 |
| 2 | One-pager / rough website copy mockup | Manik | By April 1 |
| 3 | Vet performance/social media agency, confirm availability | Manik | By end of week March 28 |
| 4 | Packaging brief updates based on 11-point feedback | Sorted (Binita) | By April 1 |
| 5 | Internal alignment on website budget range with LMC partners | Thambu / Shivendra | Before April 2 |
| 6 | Pricing decision narrowed to ₹850–900 + manufacturing cost analysis | Thambu / Shivendra | Before April 2 |

## Critical questions to capture (NEXT INGEST)

When the user is next available to update the wiki:

- Did the meeting happen on April 2 as planned?
- Was the visual identity presented? Was it approved?
- What was the website decision? (Sorted ₹6L? negotiate? external developer?)
- Was the performance/social media freelancer met? Was she a fit?
- Were packaging renders presented? Approved?
- Was [[pricing-strategy|pricing locked at ₹850-900]]?
- Did all April 2 deliverables actually land before the meeting, or were any deferred?
- What new dates / commitments came out of the meeting?

This data is the most valuable next-ingest target for the wiki. Without it, every Mindwise question has a 9-day blind spot.

## Connections
- [[launch-plan-june-1-2026]]
- [[critical-path-bottleneck]]
- [[website-development]]
- [[brand-manager-decision]]
- [[freelance-pr-strategy]]
- [[two-track-bottle-strategy]]
- [[route-3-architecture-of-intelligence]]
- [[2026-03-24-meeting-manik-pricing-website]] — where this meeting was scheduled
