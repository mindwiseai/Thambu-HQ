---
title: Product Range Expansion — Kids Bacopa, Bacopa+, Syrup
type: concept
domain: mindwise
status: CONFIRMED (planning stage)
tags: [product, range, expansion, roadmap]
---

# Product Range Expansion

Confirmed by [[shivendra-selvam]] on **March 20, 2026** in response to [[binita]]'s question about future-proofing the packaging system:

> "Thinking ahead - yes. We are working on creating a range. Bacopa for children, Bacopa + other elements and Bacopa in syrup form."

## Planned product lines

| Product | Target | Format | Status |
|---------|--------|--------|--------|
| **Mindwise (current)** | Adults | Capsules (60-cap bottle) | Market-ready |
| **Bacopa for children** | Kids | TBD (likely syrup or gummy) | Planning |
| **Bacopa+ (combined)** | Adults | Capsules (Bacopa + other elements) | Planning |
| **Bacopa syrup** | All ages | Oral liquid | Planning |
| **Gummies** | TBD | Gummy format | Formulated (BacoZest-based) |
| **Oral Disintegrating Strips** | Adults (on-the-go) | Dissolve-on-tongue strip | NDA signed Apr 2023 ([[aavishkar-oral-strips]]) |

## Gummy formulation details (from Technical Catalog)

Per the "Calculation of Bacosides in Gummies" documents:
- Each gummy: 4500mg total weight
- Contains 350mg BacoZest (17% bacosides = ~59mg bacosides/gummy)
- Recovery rate: 75.98–94.17% (considered acceptable)
- Uses BacoZest from Chemiloids (alternative to CDRI-08 extract)

> [!note]
> The gummy formulation uses **BacoZest** (Chemiloids' Bacopa extract at 17% bacosides), NOT the [[cdri-08|CDRI-08]] extract (55% bacosides). This is a different ingredient with significantly lower bacoside concentration. Marketing claims about CDRI-08 cannot be applied to the gummy format unless the formulation changes.

## Supply chain for expansion

- [[imcd]] (Mumbai) — [[beta-free-bacopa-extract|bitter-free Bacopa extract]] supplier for gummy/syrup formats
- [[solusta-puducherry]] — gummy manufacturer (already has a corporate client waiting)
- [[chemiloids-life-sciences]] (Vijayawada) — BacoZest supplier, NDA signed April 24, 2023 (also processes Terminalia chebula for BPH — a separate product)
- [[indus-life-sciences]] (Chennai) — NDA for development of gummies, tablets, capsules, oral liquids
- [[aavishkar-oral-strips]] (Hyderabad) — NDA signed April 24, 2023 for **Oral Disintegrating Strips** (dissolve-on-tongue Bacopa delivery)
- **Dr. Miltons Laboratories** (Puducherry) — current capsule contract manufacturer

## Packaging implications

[[binita]]'s insight (Mar 20): the packaging system needs to be **future-proof** — designed so new products can slot into the same visual language without a full redesign. This influenced the decision to keep the [[route-3-architecture-of-intelligence|Route 3 logo]] and [[monochrome-plus-green-pop|color system]] modular.

## Indus partnership (Dec 13, 2025)

The earliest documented expansion discussion was the December 13, 2025 meeting with [[indus-life-sciences|Indus (Vikash and Mani)]]:
- [[bacopa-combination-formulations|Combination formulations]]: Bacopa + Chlorella, Bacopa + DHA, Bacopa + Curcumin
- Bacopa-based **syrup** deliverables discussed — samples expected
- 500g Bacopa + 500g BacoZest + 100g Chlorella to be sent to [[solusta-puducherry|Solista]]
- Geographic constraint: [[southeast-asia-exclusion|7 SE Asian countries excluded for Indus]] — LMC already operates there (MemoPlus Gold, Actif Mind)

## Himani meeting (Mar 16, 2026)

Per [[himani]]'s Mar 16, 2026 meeting:
- **Bacopa for Kids** — "not now" but asked [[sorted-agency]] to create a smaller version
- **Sizing:** create both **30-cap and 60-cap** bottle versions (confirms the 60-cap D2C + 30-cap B2B split)

## Sources

- [[2026-04-13-whatsapp-full-export]] — Mar 20 entry (Shivendra's confirmation)
- [[2026-03-26-thambu-voice-brief]] — IMCD + Solista gummy/syrup path
- [[2025-11-to-2026-03-onenote-notebook]] — Dec 13 Indus meeting + Mar 16 Himani meeting
- Technical Catalog — gummy formulation details, NDA partners

## Connections

- [[cdri-08]]
- [[beta-free-bacopa-extract]]
- [[imcd]]
- [[solusta-puducherry]]
- [[semi-transparent-bottle]]
- [[binita]]
- [[shivendra-selvam]]
